#shared.py
# Description:
# This file, shared.py, is a core component responsible for managing shared resources and data for different modules in the your-pax project.
...
class SharedData:
    ...
    def get_default_config(self):
        ...
        return {
            ...
            "steal_file_extensions": [".your-pax",".hack",".flag"],
            
            "__title_bluetooth_nap__": "Bluetooth NAP Settings",
            "bluetooth_nap_ssid": "your-pax",
            "bluetooth_nap_bridge_ip": "192.168.4.1",
    ...
    def initialize_epd_display(self):
        """Initialize the e-paper display."""
        if not self.config.get("has_display", True):
            logger.info("Display disabled. Skipping EPD initialization.")
            self.width, self.height = 122, 250
            self.screen_reversed = False
            self.web_screen_reversed = False
            return
        try:
            ...
            if self.config["epd_type"] == "epd2in7":
                ...
                logger.info(f"EPD type: {self.config['epd_type']} unknown, using defaults")
                self.screen_reversed = False
                self.web_screen_reversed = False
            self.epd_helper.init_full_update()
            self.width, self.height = self.epd_helper.epd.width, self.epd_helper.epd.height
    ...
    def initialize_variables(self):
        ...
        self.your_pax_instance = None
        self.wifichanged = False
        self.bluetooth_active = False
        self.wifi_connected = False
        self.pan_connected = False
        self.usb_active = False
        self.bluetooth_nap_active = True
        self.bluetooth_nap_ip = "192.168.4.1"
        self.orchsays = "Hacking away..."
        self.orch_status = "IDLE"
        self.status_text = "IDLE"
        self.status_text2 = "Awakening..."
        ...
        self.image_series = {}
        self.manual_scan_trigger = threading.Event()
        self.manual_bruteforce_trigger = threading.Event()
        self.manual_bruteforce_protocol = "all"
        self.manual_vulnscan_trigger = threading.Event()
        self.manual_steal_trigger = threading.Event()
        self.captive_portal_class = None
    ...
    def save_config(self):
        ...
        try:
            ...
            try:
                tmpfile = self.shared_config_json + '.tmp'
                with open(tmpfile, 'w') as f:
                    json.dump(self.config, f, indent=4)
                os.replace(tmpfile, self.shared_config_json)
    ...
    def load_fonts(self):
        """Load the fonts."""
        if not self.config.get("has_display", True):
            logger.info("Display disabled. Skipping font loading.")
            return
        try:
            ...
            self.font_arialbold = self.load_font('ArialB.TTF', 12)
    ...
    def load_images(self):
        ...
        if not self.config.get("has_display", True):
            logger.info("Display disabled. Skipping image loading.")
            return
        try:
            ...
            for attr_name, filename in icon_map.items():
                img_path = os.path.join(self.staticpicdir, filename)
                setattr(self, attr_name, self.load_image(img_path))

            self.yourpax1 = self.load_image(os.path.join(self.staticpicdir, 'yourpax1.bmp'))
            self.x_center1 = (self.width - self.yourpax1.width) // 2
            self.y_bottom1 = self.height - self.yourpax1.height

            if os.path.isdir(self.statuspicdir):
                for status_name in os.listdir(self.statuspicdir):
                    status_dir = os.path.join(self.statuspicdir, status_name)
                    if os.path.isdir(status_dir):
                        images = []
                        for img_file in sorted(os.listdir(status_dir)):
                            if img_file.lower().endswith(('.bmp', '.png')):
                                img_path = os.path.join(status_dir, img_file)
                                img = self.load_image(img_path)
                                if img:
                                    images.append(img)
                        if images:
                            self.image_series[status_name] = images
    ...
    def update_status_image(self):
        """ Using getattr to obtain the reference of the attribute with the name stored in self.orch_status"""
        try:
            self.status_image = getattr(self, self.orch_status)
            if self.status_image is None:
                raise AttributeError
        except AttributeError:
            logger.warning(f"The image for status {self.orch_status} is not available, using IDLE image by default.")
            self.status_image = self.attack
        
        self.status_text = self.orch_status  # Mettre à jour le texte du statut
    ...
    def update_stats(self):
        ...
        self.levelnbr = int((self.networkkbnbr * 0.1 + self.crednbr * 0.2 + self.datanbr * 0.1 + self.zombiesnbr * 0.5+ self.attacksnbr * 1 + self.vulnnbr * 0.01 ))
