...
manage_wifi_connections() {
    ...
    if [ -f "$PRECONFIG_FILE" ]; then
        ...
        if [ -z "$SSID" ]; then
            ...
            echo "SSID not found in preconfigured Wi-Fi file. Check the log for details."
        ...
        if nmcli connection add type wifi ifname wlan0 con-name "$SSID" ssid "$SSID" \
            ...
            echo "Failed to create Wi-Fi connection for SSID: $SSID. Check the log for details."
