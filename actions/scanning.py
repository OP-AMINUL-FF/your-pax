...
class NetworkScanner:
    ...
    class ScanPorts:
        ...
        def start(self):
            ...
            print(f"Scanning {len(self.ip_data.ip_list)} IPs...")
    ...
    class LiveStatusUpdater:
        ...
        def read_csv(self):
            """
            Reads the source CSV file into a list of dicts.
            """
            try:
        ...
        def calculate_open_ports(self):
            ...
            """
            try:
                alive_rows = [row for row in self.df if row.get('Alive') == '1']
        ...
        def calculate_hosts_counts(self):
            ...
            """
            try:
                self.all_known_hosts_count = len([row for row in self.df if row.get('MAC Address') != 'STANDALONE'])
                self.alive_hosts_count = len([row for row in self.df if row.get('Alive') == '1'])
            except Exception as e:
            ...
            try:
                if os.path.exists(self.output_csv_path):
                    with open(self.output_csv_path, 'r') as file:
                        ...
                        rows = list(reader)
                    if rows:
                        for col in ['Total Open Ports', 'Alive Hosts Count', 'All Known Hosts Count']:
                            if col not in fieldnames:
                                fieldnames = fieldnames + [col]
                        rows[0]['Total Open Ports'] = str(self.total_open_ports)
                        rows[0]['Alive Hosts Count'] = str(self.alive_hosts_count)
                        rows[0]['All Known Hosts Count'] = str(self.all_known_hosts_count)
    ...
    def scan(self):
        ...
        try:
            ...
            self.shared_data.status_text2 = str(network)
            ...
            print("Scan Results:")
            print("-" * 80)
            header = ["IP", "Hostname", "Alive", "MAC Address"] + [str(port) for port in all_ports]
            print(" | ".join(header))
            ...
            for ip, ports, hostname, mac in zip(ip_data.ip_list, open_ports.values(), ip_data.hostname_list, ip_data.mac_list):
                ...
                row = [ip, hostname, alive, mac] + [str(port) if port in ports else "" for port in all_ports]
                print(" | ".join(row))
