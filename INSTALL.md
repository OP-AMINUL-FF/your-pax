...
- Username and hostname set to `your-pax`.
...
I did not develop your-pax for the raspberry pi zero w2 64bits, but several feedbacks have attested that the installation worked perfectly.
...
The fastest way to install your-pax is using the automatic installation script :
...
#### Step 3: your-pax Installation

```bash
# Clone the your-pax repository
cd  # go to your home directory (or wherever you want to clone)
...
```bash
sudo vi ~/your-pax/config/shared_config.json
```
...
##### 7.1: your-pax Service

Create the service file:

```bash
sudo vi /etc/systemd/system/your-pax.service
```
...
[Service]
ExecStartPre=__YOUR_PAX_DIR__/kill_port_8000.sh
ExecStart=/usr/bin/python3 __YOUR_PAX_DIR__/your-pax.py
WorkingDirectory=__YOUR_PAX_DIR__
...
ExecStartPost=/bin/bash -c 'FILE_LIMIT=$(ulimit -n); THRESHOLD=$(( FILE_LIMIT - 1000 )); while :; do TOTAL_OPEN_FILES=$(lsof | wc -l); if [ "$TOTAL_OPEN_FILES" -ge "$THRESHOLD" ]; then echo "File descriptor threshold reached: $TOTAL_OPEN_FILES (threshold: $THRESHOLD). Restarting service."; systemctl restart your-pax.service; exit 0; fi; sleep 10; done &'
...
```bash
chmod +x ~/your-pax/kill_port_8000.sh
```
...
1. Power on the your-pax device (Bluetooth advertises as **`your-pax`**).
2. Open the app — `SplashScreen` scans for nearby Bluetooth devices.
3. Select **`your-pax`** → the app pairs automatically.
4. The app opens an RFCOMM Bluetooth socket (SPP) to the device's serial server.
5. The app fetches the CSRF token over the serial link and lands you on the **Home** screen.

Everything — dashboard, network scan, Wi-Fi attacks, Evil AP, loot, store, config, backup, manual mode, live EPD, logs — is now controllable from your phone over the Bluetooth SPP connection. (For **Web Only** mode, the browser accesses the device over Bluetooth PAN at `http://192.168.4.1:8000`.)
